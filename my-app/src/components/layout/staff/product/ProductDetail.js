            ...
            <Row>
                <Col md={5}>
                    ...
                    {productDetailSelected &&
                        <img src={productDetailSelected.url} />
                    }
                </Col>
                <Col md={7}>
                    <Row>
                        ...
                        <Col md={12} style={{ textAlign: "left" }}>
                            <h5><span style={{ marginRight: "5%" }}>Color</span>
                                {productDetails.map((item) => {
                                    if (item.status === 1) {
                                        return <button key={item.id} style={{ marginLeft: "2%" }} onClick={(e) => { handleClickColor(e, item) }} onMouseOver={() => handleHoveColor(item)} onMouseLeave={() => handleOutHoveColor()} value={item.id} className="btn-color">{item.label}</button>
                                    } else {
                                        return <button key={item.id} className="btn-color" style={{ marginLeft: "2%", borderColor: 'white', color: '#b6b6b6fe' }} disabled >{item.label}</button>
                                    }
                                })}
                            </h5>
                        </Col>
                        <Col md={12} style={{ textAlign: "left" }}>
                            <h5><span style={{ marginRight: "5%" }}>Size</span>
                                {sizes.map((item) => {
                                    if (item.status === true) {
                                        return <button key={item.value} style={{ marginLeft: "2%" }} onClick={(e) => { handleClickSize(e, item.value) }} value={item.value} className="btn-size">{item.label}</button>
                                    } else {
                                        return <button key={item.value} className="btn-size" style={{ marginLeft: "2%", borderColor: 'white', color: '#b6b6b6fe' }} disabled >{item.label}</button>
                                    }
                                })}
                            </h5>
                        </Col>
